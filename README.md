...
## Claude Code로 직접 사용하기

Claude Code 환경에서 자연어로 SAP를 생성할 수도 있습니다:
...
# 다음과 같이 요청:
> YH-SAP 파이프라인으로 프로토콜 파일에서 SAP를 생성해줘.
> 프로토콜 파일: ../YH#####-###_Clinical_Protocol.docx
> 테스트 모드로 먼저 실행하고, 결과 확인 후 sonnet으로 재실행해줘.
```

또는 직접 명령:

```bash
# Claude Code에서 한 줄로 실행
claude -p "cd sap-kcl && source ../YH-SAP-venv/bin/activate && python -m WriteSAPs.yuhan_sap_json ../프로토콜.docx --test"
```
